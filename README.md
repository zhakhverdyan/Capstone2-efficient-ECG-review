...
<img src="Images/114_plot.png" width="500"><img src="Images/114_zoomed.png" width="500">

**Goal:** To streamlineing ECG record review by providing arrhythmic beat predictions.

**Background:** Arrhythmia diagnosis often requires doctors to review 24-48 h of ECG recordings where most of the beats are normal. This is extremely time and labor-intensive. This tool aims to reduce the review time by automatically labeling the most likely abnormal beats. There are 5 major classes of heartbeats - N - normal, S - supreraverntricular ectopic beat, V - ventricular ectopic beat, F - fusion between normal and ventricular, and Q - unclassifiable.

**Approach:** Deep learning with convolutional neural network

**Dataset:** MIT-BIH Arrhytmia dataset available at https://physionet.org/content/mitdb/1.0.0/

**Challenges:** Abnormal beats account for a minority of >90k labeled beats in the dataset. Moreover, they are unevenly distributed. A useful model should successfully identify abnormal beats in unseen patients, hence the train, validation and test split has to be performed on records as apposed to beats. So in some sense there are only 44 distinct samples, and classes S and F are poorly represented (I excluded class Q (n=15) prior to analysis). In addition, there are technical artifacts (see feature engineering).
...
QRS complex is a prominent feature used to identify heartbeats. In the MIT-BIH-Arrhythmia dataset the beat annotation coordinates coincide with the QRS peak. Since this project is not aboout heartbeat segmentation I split up the signal based on beat label coordinates (+/- 0.5 sec for each annotation). Since the signal frequency is 360Hz and I took 1 second/beat the final feature number was 360.

<img src="Images/canonical_heartbeat.png" width="400"><img src="Images/Sample_record_100.png" width="400">

To overcome some of the technical artifacts, i.e. baseline drift and polarity inversion I took the absolute value of the signal and then applied the dif function, which simply takes the difference of two consecutive points. In this way only the biggest changes in the signal are revealed and all the minor ones, such as baseline drift, are silenced.
...
**Deep learning**
[Data exploration](Jupyter_notebooks/Model_fitting/train_val_test_systematic_CNN_fits.ipynb)
[Data exploration](Jupyter_notebooks/Model_fitting/CNN_best_model_evaluation.ipynb)
...
The model has high variance which is expected given the high inter-record variation and small number of distinct records.

Here is the confusion matrix for all classes:

<img src="Images/best_model_conf_matrix.png" width="250">

Although the recall for classes S anf F are small, additional 22% of S class and 28% of F class would be detected whn reviewing V class predictions. Summary classification report:
